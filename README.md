...
| 🔐 **Auth System** | Login / Sign-Up with SHA-256 hashed passwords (SQLite) |
...
Custom model weights are stored at:
```
runs/detect/train_extended/weights/best.pt
```

The app runs in **Auto Hybrid** mode by default — uses the custom model if it detects objects, otherwise falls back to the base YOLOv11 model.
...
| Total Images | 9 annotated images |
...
```bash
git clone https://github.com/divyadoredla/-AI-Vehicle-Monitoring-App.git
cd -AI-Vehicle-Monitoring-App
```
...
> The default admin account is auto-created on first boot. You can also sign up with any new username via the **Sign Up** tab.
...
Training uses `train/` and `valid/` splits from `data.yaml`. Results are saved to `runs/detect/train_extended/`.
...
Create a new Space → select **Streamlit** → push your code → live instantly!
...
Upload Image / Video
        │
        ▼
Night Mode? → Yes → CLAHE Low-Light Enhancement
        │
        ▼
Model Selection (Auto Hybrid / Base Model / Custom Precision)
        │
        ▼
YOLOv11 Inference
        │
        ├── Draw Bounding Boxes  (orange = normal, red = breach/speeding)
        ├── Label: Vehicle class + speed (video) or confidence (image)
        ├── Save annotated result to saved_detections/
...
- Passwords are **SHA-256 hashed** before being stored
- No plain-text passwords are ever saved
- SQLite database is local (ephemeral on cloud — resets on restart)
- For production use, replace SQLite with a persistent DB (PostgreSQL, Supabase, etc.)
...
Model weights and application code are for educational and research use.
